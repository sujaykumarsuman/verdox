...
```
 ...
 | avatar_url   |   |    | default_branch   |   |   | timeout_seconds|   |
 | created_at   |   |    | local_path       |   |   | created_at     |   |
 | updated_at   |   |    | clone_status     |   |   | updated_at     |   |
 +------+-------+   |    | is_active        |   |   +-------+--------+   |
        |            |    | created_at       |   |           |            |
        |            |    | updated_at       |   |           |            |
        |            |    +--------+---------+   |           |            |
 ...
 +------------- +   |             |              |    | started_at    |  |
                     ...
                     +----+-user_id (FK)     |   |    |------------------|
                     |    | team_id (FK)-----+---+    | id (PK)          |
                     +----+-invited_by (FK)  |   |    | repository_id(FK)|
                          | id (PK)          |   |    | discovery_json   |
                          | role             |   |    | scripts_path     |
                          | status           |   |    | discovered_by(FK)|
                          | created_at       |   |    | created_at       |
                          +--------+---------+   |    +------------------+
                                   |             |                       |
                          +--------v---------+   |   +-----v-----------------+
                          | team_join_requests|   |   |   teams               |
                          |------------------|   |   |------------------------|
                          | id (PK)          |   |   | id (PK)                |
                          | team_id (FK)     |   |   | name                   |
                          | user_id (FK)     |   |   | slug                   |
                          | message          |   +---+ created_by             |
                          | status           |       | github_pat_encrypted   |
                          | reviewed_by (FK) |       | github_pat_nonce       |
                          | role_assigned    |       | github_pat_set_at      |
                          | created_at       |       | github_pat_set_by (FK) |
                          | updated_at       |       | github_pat_github_user |
                          +------------------+       | created_at             |
                                                     | updated_at             |
                                                     | deleted_at             |
                                                     +------------------------+
...
| repositories 1 : N test_discoveries   | 1:N  | test_discoveries.repository_id |
...
> and a background job should be queued to asynchronously delete local clones
> from disk (`verdox:jobs:cleanup`). This cascade is handled at the
> application layer, not via database triggers.
...
```
migrations/
  000001_create_enum_types.up.sql          # updated enums
  000001_create_enum_types.down.sql
  000002_create_users.up.sql
  000002_create_users.down.sql
  000003_create_sessions.up.sql
  000003_create_sessions.down.sql
  000004_create_repositories.up.sql        # updated (local_path, clone_status, no owner_id)
  000004_create_repositories.down.sql
  000005_create_teams.up.sql               # includes team-level PAT columns
  000005_create_teams.down.sql
  000006_create_team_members.up.sql        # updated enum references
  000006_create_team_members.down.sql
  000007_create_team_join_requests.up.sql   # NEW
  000007_create_team_join_requests.down.sql
  000008_create_team_repositories.up.sql
  000008_create_team_repositories.down.sql
  000009_create_test_suites.up.sql
  000009_create_test_suites.down.sql
  000010_create_test_runs.up.sql           # updated (run_number)
  000010_create_test_runs.down.sql
  000011_create_test_results.up.sql
  000011_create_test_results.down.sql
  000012_create_test_discoveries.up.sql    # NEW
  000012_create_test_discoveries.down.sql
```
...
### 000004 -- create_repositories

**UP** `migrations/000004_create_repositories.up.sql`
...
**DOWN** `migrations/000004_create_repositories.down.sql`
...
**UP** `migrations/000005_create_teams.up.sql`
...
**DOWN** `migrations/000005_create_teams.down.sql`
...
### 000006 -- create_team_members

**UP** `migrations/000006_create_team_members.up.sql`
...
**DOWN** `migrations/000006_create_team_members.down.sql`
...
### 000007 -- create_team_join_requests

**UP** `migrations/000007_create_team_join_requests.up.sql`
...
**DOWN** `migrations/000007_create_team_join_requests.down.sql`
...
### 000008 -- create_team_repositories

**UP** `migrations/000008_create_team_repositories.up.sql`
...
**DOWN** `migrations/000008_create_team_repositories.down.sql`
...
### 000009 -- create_test_suites

**UP** `migrations/000009_create_test_suites.up.sql`
...
**DOWN** `migrations/000009_create_test_suites.down.sql`
...
### 000010 -- create_test_runs

**UP** `migrations/000010_create_test_runs.up.sql`
...
**DOWN** `migrations/000010_create_test_runs.down.sql`
...
### 000011 -- create_test_results

**UP** `migrations/000011_create_test_results.up.sql`
...
**DOWN** `migrations/000011_create_test_results.down.sql`
...
### 000012 -- create_test_discoveries

**UP** `migrations/000012_create_test_discoveries.up.sql`
...
**DOWN** `migrations/000012_create_test_discoveries.down.sql`

```sql
DROP TABLE IF EXISTS test_discoveries;
```
